...
const ProductCard = ({ product }) => {
  ...
  // Discount is admin-entered and applied directly to the selling price.
  // Final = sellingPrice * (1 - discount/100). When no discount, final = sellingPrice.
  const discountPercent = Number(product.discountPercent) || 0;
  const hasDiscount = discountPercent > 0;
  const finalPrice = hasDiscount
    ? Math.round(sellingPrice * (1 - discountPercent / 100))
    : sellingPrice;
  const savings = hasDiscount ? sellingPrice - finalPrice : 0;
  ...
  return (
    <div className="group bg-card rounded-xl border border-border overflow-hidden shadow-card hover:shadow-lg transition-all">
      ...
      <div className="p-4">
        ...
        {/* Price line: strikethrough selling price + discounted final price beside it */}
