...
export const productService = {
  ...
  // Tags flagged to appear in the customer navbar/footer (backend caps at 3).
  async listNavbarTags() {
    const response = await apiRequest("/tags?navbar=true");
    return (response?.items || []).map((t) => ({ id: t.id, name: t.name, icon: t.icon || null }));
  },
};
